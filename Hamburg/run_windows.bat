@echo off
set SCRIPT_DIR=%~dp0
set TARGET=%SCRIPT_DIR%..\..\cities\data\HAM
set VERSION=1.30.0
echo [Hamburg Mod] Copying data files to cities\data\HAM...
if not exist "%TARGET%" mkdir "%TARGET%"
copy /Y "%SCRIPT_DIR%data\HAM\*" "%TARGET%\" >nul
echo [Hamburg Mod] Data files copied successfully.
if not exist "%SCRIPT_DIR%pmtiles.exe" (
    echo [Hamburg Mod] Downloading pmtiles.exe...
...
echo [Hamburg Mod] Starting tile server on port 8080...
