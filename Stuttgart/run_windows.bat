@echo off
set SCRIPT_DIR=%~dp0
set TARGET=%SCRIPT_DIR%..\..\cities\data\STU
set VERSION=1.30.0
echo [Stuttgart Mod] Copying data files to cities\data\STU...
if not exist "%TARGET%" mkdir "%TARGET%"
copy /Y "%SCRIPT_DIR%data\STU\*" "%TARGET%\" >nul
echo [Stuttgart Mod] Data files copied successfully.
if not exist "%SCRIPT_DIR%pmtiles.exe" (
    echo [Stuttgart Mod] Downloading pmtiles.exe...
...
echo [Stuttgart Mod] Starting tile server on port 8080...
