@echo off
set SCRIPT_DIR=%~dp0
set TARGET=%SCRIPT_DIR%..\..\cities\data\CHZ
set VERSION=1.30.0
echo [Chemnitz Mod] Copying data files to cities\data\CHZ...
if not exist "%TARGET%" mkdir "%TARGET%"
copy /Y "%SCRIPT_DIR%data\CHZ\*" "%TARGET%\" >nul
echo [Chemnitz Mod] Data files copied successfully.
if not exist "%SCRIPT_DIR%pmtiles.exe" (
    echo [Chemnitz Mod] Downloading pmtiles.exe...
...
echo [Chemnitz Mod] Starting tile server on port 8080...
