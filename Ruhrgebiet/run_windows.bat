@echo off
set SCRIPT_DIR=%~dp0
set TARGET=%SCRIPT_DIR%..\..\cities\data\RHR
set VERSION=1.30.0
echo [Ruhrgebiet Mod] Copying data files to cities\data\RHR...
if not exist "%TARGET%" mkdir "%TARGET%"
copy /Y "%SCRIPT_DIR%data\RHR\*" "%TARGET%\" >nul
echo [Ruhrgebiet Mod] Data files copied successfully.
if not exist "%SCRIPT_DIR%pmtiles.exe" (
    echo [Ruhrgebiet Mod] Downloading pmtiles.exe...
...
echo [Ruhrgebiet Mod] Starting tile server on port 8080...
