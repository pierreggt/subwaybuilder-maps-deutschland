@echo off
set SCRIPT_DIR=%~dp0
set TARGET=%SCRIPT_DIR%..\..\cities\data\BIE
set VERSION=1.30.0
echo [Bielefeld Mod] Copying data files to cities\data\BIE...
if not exist "%TARGET%" mkdir "%TARGET%"
copy /Y "%SCRIPT_DIR%data\BIE\*" "%TARGET%\" >nul
echo [Bielefeld Mod] Data files copied successfully.
if not exist "%SCRIPT_DIR%pmtiles.exe" (
    echo [Bielefeld Mod] Downloading pmtiles.exe...
...
echo [Bielefeld Mod] Starting tile server on port 8080...
