@echo off
set SCRIPT_DIR=%~dp0
set TARGET=%SCRIPT_DIR%..\..\cities\data\AAC
set VERSION=1.30.0
echo [Aachen Mod] Copying data files to cities\data\AAC...
if not exist "%TARGET%" mkdir "%TARGET%"
copy /Y "%SCRIPT_DIR%data\AAC\*" "%TARGET%\" >nul
echo [Aachen Mod] Data files copied successfully.
if not exist "%SCRIPT_DIR%pmtiles.exe" (
    echo [Aachen Mod] Downloading pmtiles.exe...
...
echo [Aachen Mod] Starting tile server on port 8080...
