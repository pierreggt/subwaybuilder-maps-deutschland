@echo off
set SCRIPT_DIR=%~dp0
set TARGET=%SCRIPT_DIR%..\..\cities\data\ERF
set VERSION=1.30.0
echo [Erfurt Mod] Copying data files to cities\data\ERF...
if not exist "%TARGET%" mkdir "%TARGET%"
copy /Y "%SCRIPT_DIR%data\ERF\*" "%TARGET%\" >nul
echo [Erfurt Mod] Data files copied successfully.
if not exist "%SCRIPT_DIR%pmtiles.exe" (
    echo [Erfurt Mod] Downloading pmtiles.exe...
...
echo [Erfurt Mod] Starting tile server on port 8080...
